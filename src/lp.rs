...
pub trait LpCostFunction<L: Language, N: Analysis<L>> {
    ...
    /// of the given e-node.
    fn node_cost(&mut self, egraph: &EGraph<L, N>, eclass: Id, enode: &L, cells: &Vec<(String, f64, f64, String)>) -> f64;
}

fn try_convert<L: Language>(node: &L) -> Option<&SimpleLanguage> {
    node.as_any().downcast_ref::<SimpleLanguage>()
}



impl<L: Language, N: Analysis<L>> LpCostFunction<L, N> for AstSize {
    fn node_cost(&mut self, _egraph: &EGraph<L, N>, _eclass: Id, _enode: &L, cells: &Vec<(String, f64, f64, String)>) -> f64 {
        if let Some(simple_node) = try_convert(_enode) {
            let (node_size, _) = match simple_node {
                SimpleLanguage::Symbol(_) => (0.0, 0.0),
                SimpleLanguage::OUT(_) => (0.0, 0.0),
                _ => {
                    let node_type = get_node_type(simple_node);
                    cells.iter()
            ...
            println!("{:?}",_enode);
            panic!("Conversion failed. `node` is not a `SimpleLanguage`.");
        }
        // println!("{:?}",simple_node);
    }
}
    ...
    N: Analysis<L>,
{
    ...
    pub fn new<CF>(egraph: &'a EGraph<L, N>, mut cost_function: CF, cells: Vec<(String, f64, f64, String)>) -> Self
    where
        CF: LpCostFunction<L, N>,
    {
        ...
        for class in egraph.classes() {
            for (node, &node_active) in class.iter().zip(&vars[&class.id].nodes) {
                model.set_obj_coeff(node_active, cost_function.node_cost(egraph, class.id, node,&cells));
            }
        }
    ...
    pub fn solve_multiple(&mut self, roots: &[Id]) -> RecExpr<L> {
